...
リファクタリング ベースのアプローチの利点は、開発のために並列の繰り返しパスを作成できることです。 下の図は、2 つの並列繰り返しパスの例を示しています。クラウド導入とクラウド プラットフォームです。 どちらも独自のペースで進行するので、いずれかのチームの日々の取り組みに対する阻害要因となるリスクが最小限になります。 導入計画とリファクタリング ガードレールに関する整合により、マイルストーンについて一連の同意が得られます。これにより、将来の状態の依存関係が明確になります。
