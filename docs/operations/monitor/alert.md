...
[管理ソリューション](/azure/azure-monitor/insights/solutions-inventory)は一般に、データを Azure ログ ストアに格納します。 2 つの例外は、Azure Monitor for VMs とコンテナーに対する Azure Monitor です。 次の表では、特定のデータ型とその格納場所に基づくアラートのエクスペリエンスについて説明します。
