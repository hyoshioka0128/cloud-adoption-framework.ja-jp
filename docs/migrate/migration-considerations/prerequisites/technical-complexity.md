...
_スプリント バックログ_、または_イテレーション バックログ_ は、個々の資産の移行を処理する、単一のスプリントまたはイテレーションで完了する技術的な作業で構成されています。 その作業は、移行されるワークロードのリストから派生される必要があります。 プロジェクト管理で Azure DevOps (以前の Visual Studio Online) などのツールを使用する場合、スプリントの作業項目は、リリース バックログの製品バックログ項目の子となり、移行バックログのエピックになります。 このような親と子の関係により、すべてのレベルで変更管理がわかりやすくなります。
