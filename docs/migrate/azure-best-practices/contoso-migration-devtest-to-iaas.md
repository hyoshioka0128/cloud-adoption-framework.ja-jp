...
進行中の開発をサポートするために、Contoso は既存の VM を引き続き使用し、それらを Azure に移行することにしました。 今後、Contoso は [Azure SQL Database](/azure/azure-sql/azure-sql-iaas-vs-paas-what-is-overview) や [Azure Database for MySQL](/azure/mysql/overview) などのサービスとしてのプラットフォーム (PaaS) サービスの使用を推進します。
...
> [!NOTE]
> Contoso は、[DevTest Labs](/azure/devtest-labs/devtest-lab-overview) を使用することで、この一覧の短所に対処できます。
...
[移行のために VM を準備する](/azure/migrate/prepare-for-migration)方法を参照してください。
...
[テスト移行を実行する](/azure/migrate/tutorial-migrate-vmware#run-a-test-migration)方法、および [VM を Azure に移行する](/azure/migrate/tutorial-migrate-vmware#migrate-vms)方法を参照してください。
...
事業継続とディザスター リカバリーのために、Contoso はデータを保護します。 Contoso は、Azure Backup サービスを使用して VM 上のデータをバックアップします。 詳細については、「[Azure VM バックアップの概要](/azure/backup/backup-azure-vms-introduction)」をご覧ください。
