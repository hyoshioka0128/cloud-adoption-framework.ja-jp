...
- **ゾーン マップ**: Netezza では、一部の列の型に対して、ゾーン マップが自動的に作成されて維持されます。 ゾーン マップは、スキャンされるデータの量を制限するために、次の列の型に対するクエリ時に使用されます。
  ...
  - `CHAR` 列 (それらがマテリアライズドビューの一部であり、`ORDER BY` 句に含まれる場合)
...
- **マテリアライズドビュー**: Netezza では、多数の列があり、そのうちの数列だけがクエリでよく使用される大きなテーブルより、1 つ以上のマテリアライズドビューを作成することが推奨されます。 マテリアライズドビューは、ベース テーブルのデータが更新されるとシステムで自動的に維持されます。

  現在、Azure Synapse では、Netezza と同じ機能を持つマテリアライズドビューのプレビュー サポートが提供されています。
