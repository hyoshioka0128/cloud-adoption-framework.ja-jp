...
サービスをオンプレミスに維持する場合でも、Azure を使用して、オンプレミス リソースのすべてまたは一部の側面の管理を段階的に開始できます。 また、ビジネスで採用されているクラウド サービス モデルに基づいて、ビジネスのクラウド導入戦略に合わせて、責任の監視範囲も戦略で定義します。 IaaS に基づくサービスの場合でも、Azure Service Health によりメトリック、ログ、ビュー、アラートの機能を利用できます。ここでは、リソース正常性を使用した Azure リソースの可用性監視からアラートを構成します。 Microsoft 365 などの SaaS サービスでは、多くは既に提供されており、ポータル、ダッシュボード、分析、アラートへの適切なアクセスを構成する必要があります。 サービスの観点から見ると、Microsoft 365 Exchange Online などの分散コンポーネントを含む大規模なサービスには、その正常性と状態を監視する必要があるだけでなく、さまざまな目標があります。
