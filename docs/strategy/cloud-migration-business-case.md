...
- Azure などのクラウド プロバイダーでは、クラウドへの投資を見積もる計算ツールが提供されています。 [Azure 料金計算ツール](https://azure.microsoft.com/pricing)は 1 つの例です。
