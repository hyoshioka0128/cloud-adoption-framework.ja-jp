...
- **Azure ロールベースのアクセス制御の使用。** Azure ロールベースのアクセス制御 (Azure RBAC) でグループを使用して管理されていないサブスクリプション、リソース グループ、または個々のリソースの数。
...
- **Azure RBAC のトリガー。** Azure ロールベースのアクセス制御方法を使用するリソースが *x%* を下回る会社では、リソースへのアクセス権をユーザーに割り当てる最適化された方法を特定するために、ID ベースライン規範への投資を検討する必要があります。
