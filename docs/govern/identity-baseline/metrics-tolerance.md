...
- **ディレクトリ サービスの複雑さのトリガー。** 管理している個々のフォレスト、ドメイン、またはディレクトリ テナントの数が _x_ より多い会社では、アカウント管理に関するリスクと、複数のシステムに散在している複数のユーザー資格情報に関する効率性の問題を軽減するために、ID ベースライン規範への投資が必要です。
