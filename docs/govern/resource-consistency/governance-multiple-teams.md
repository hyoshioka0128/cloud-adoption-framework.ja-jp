...
- **最小限の特権のアクセス許可モデル**。既定では、アクセス許可がユーザーに付与されていません。 このモデルは以下をサポートする必要があります。
  ...
  - この企業は 3 つの環境それぞれで個別にロールを管理しなければならない状況を望んでいないため、Azure のロールベースのアクセス制御 (RBAC) で使用できる[組み込みロール](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles)のみを使用する必要がある。 この企業がカスタム RBAC ロールを使用した場合、3 つの環境でカスタム ロールを同期させるために追加のプロセスが必要になる。
